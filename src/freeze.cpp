...
void evolve::ini(Fluid* grid,double t0,double maxtime,double dtauin){
  maxx=grid->getmaxX(); maxy=grid->getmaxY(); maxeta=grid->getmaxZ();
  nnx=(grid->Get_nx()+1)/2;                                                          //******
  nny=(grid->Get_ny()+1)/2;                                                          //******
  nneta=(grid->Get_nz()+1)/2;                                                        //******
...
void evolve::gethypersurface(Fluid* grid, EoS* EOS,double Tf){
  Tfreeze=EOS->temperature(Tf,0,0,0);
  cout<<"calculating freeze-out hypersurface at Tf="<<Tfreeze<<endl;

 // double Efreeze=EOS->ET(Tfreeze);
  double Efreeze =Tf;                                                                    //******        freezeout according to energy density
...
/*                                                                               //******
  etafreeze=etavis(EOS->ET(Tf),EOS,grid)/EOS->SE(Efreeze);
  grid->trcoeff->getEta(EOS->ET(Tf),Tf,etaovs,zetaovs);
  cout<<"calculating freeze-out hypersurface at etaf="<<etafreeze<<endl;
  cs2freeze=EOS->DPE(Efreeze);
  cout<<EOS->DPE(Efreeze)<<endl;
  cout<<"calculating freeze-out hypersurface at cs2="<<cs2freeze<<endl;
  double entalpyfreeze=Efreeze+EOS->PE(Efreeze);
  double pressfreeze=EOS->PE(Efreeze);
...
void evolve::put(int dumpstep,Fluid* h,double tau,EoS* EOS){
  ...
  for (int i=0;i<nnx;i++){int ii=2*i;
	...
	pixxmap[dumpstep]->put(0,i,j,k);                                              //******
	pixymap[dumpstep]->put(0,i,j,k);                                              //******
	//o=c->getpi(1,3)*cosh(eta)+c->getpi(0,1)*sinh(eta);
	pixemap[dumpstep]->put(0,i,j,k);                                              //******
	piyymap[dumpstep]->put(0,i,j,k);                                              //******
	//o=c->getpi(2,3)*cosh(eta)+c->getpi(0,2)*sinh(eta);
	piyemap[dumpstep]->put(0,i,j,k);                                              //******
	//o=c->getpi(3,3)*cosh(eta)*cosh(eta)
	//  +c->getpi(0,0)*sinh(eta)*sinh(eta)+
	//  c->getpi(0,3)*sinh(2.*eta);
	pieemap[dumpstep]->put(0,i,j,k);                                               //******
	bulkmap[dumpstep]->put(0,i,j,k);                                               //******
      }
    }
 }
 /* int ic=(h->getNX()-1)/2,jc=(h->getNY()-1)/2,kc=(h->getNZ()-1)/2;                   //******
  // cout<<i<<" "<<j<<" "<<k<<endl;
  Cell *c = h->getCell(ic,jc,kc);
  double   eta=h->getZ(kc);//eta=0;	
...
double evolve::etavis(double en,EoS* EOS,Fluid* grid){
  // return 1e-10;
  ...
  // else{
 // double etaS, zetaS;
 // grid->trcoeff->getEta(en,EOS->TE(en),etaS,zetaS);                           //******
   return 0;
   //  }
  // return par::alpha *EOS.se(en);
}

double evolve::zetavis(double en,EoS* EOS,Fluid* grid){
  // return 1e-10;
  //   return minimum(3.* EOS.pe(en) *t0/4./par::ren,par::alpha *EOS.se(en))/
  //     (1.+exp(-(EOS.te(en)-par::Tmintau)/par::Tsc));
  //   return par::alpha *EOS.se(en)/
...
void evolve::setebe(Fluid *f, EoS *eos, double tau0) {
...
void evolve::writefinal(Fluid *f,EoS *eos,double eta,double t0,char* dirname){
...
void evolve::writespacetimehistory(Fluid *f,EoS *eos,double t0,char* dirname){
