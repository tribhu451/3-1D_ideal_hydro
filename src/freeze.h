...
class evolve{
 public:
  ...
  double etavis(double en, EoS* EOS,Fluid* grid);
  void ini(Fluid* grid,double tau0,double maxtime,double dtauin);
  double zetavis(double en,EoS* EOS, Fluid* grid);
  ...
  void gethypersurface(Fluid* grid, EoS* EOS,double Tf);
  void put(int step,Fluid* grid,double tau,EoS* EOS);
...
void writeVector3D(std::ofstream& oh,const char* name,const char* unit,
  ...
  void setebe(Fluid *f, EoS *eos, double tau0) ;
  void   writefinal(Fluid *f,EoS *eos,double eta,double t0,char* dirname);
  void   writespacetimehistory(Fluid *f,EoS *eos,double t0,char* dirname);
 char system[10];
  char device[10];
